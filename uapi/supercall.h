...
/*
 * After each ksu_sepolicy_cmd_hdr, command arguments are encoded sequentially
 * as: [u32 len][len bytes][\0], where len excludes the trailing '\0'. len == 0
 * represents ALL. Argument count is derived from cmd:
...
DEFINE_KSU_UAPI_CONST(__u8, KSU_UMOUNT_WIPE,
                      0) /* ignore everything and wipe list */
...
// Downstream add mode
DEFINE_KSU_UAPI_CONST(__u8, KSU_UMOUNT_GETSIZE_LEGACY,
                      107) // get list size (legacy)
DEFINE_KSU_UAPI_CONST(__u8, KSU_UMOUNT_GETLIST_LEGACY, 108) // get list (legacy)
DEFINE_KSU_UAPI_CONST(__u8, KSU_UMOUNT_GETSIZE_NEW,
                      200) // get list size (new (with flags))
DEFINE_KSU_UAPI_CONST(__u8, KSU_UMOUNT_GETLIST_NEW,
                      201) // get list (new (with flags))
...
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_REPORT_EVENT,
                      _IOC(_IOC_WRITE, 'K', 3, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_SET_SEPOLICY,
                      _IOC(_IOC_READ | _IOC_WRITE, 'K', 4, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_CHECK_SAFEMODE,
                      _IOC(_IOC_READ, 'K', 5, 0))
// deprecated
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_GET_ALLOW_LIST,
                      _IOC(_IOC_READ | _IOC_WRITE, 'K', 6, 0))
// deprecated
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_GET_DENY_LIST,
                      _IOC(_IOC_READ | _IOC_WRITE, 'K', 7, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_NEW_GET_ALLOW_LIST,
                      _IOWR('K', 6, struct ksu_new_get_allow_list_cmd))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_NEW_GET_DENY_LIST,
                      _IOWR('K', 7, struct ksu_new_get_allow_list_cmd))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_UID_GRANTED_ROOT,
                      _IOC(_IOC_READ | _IOC_WRITE, 'K', 8, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_UID_SHOULD_UMOUNT,
                      _IOC(_IOC_READ | _IOC_WRITE, 'K', 9, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_GET_MANAGER_APPID,
                      _IOC(_IOC_READ, 'K', 10, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_GET_APP_PROFILE,
                      _IOC(_IOC_READ | _IOC_WRITE, 'K', 11, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_SET_APP_PROFILE,
                      _IOC(_IOC_WRITE, 'K', 12, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_GET_FEATURE,
                      _IOC(_IOC_READ | _IOC_WRITE, 'K', 13, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_SET_FEATURE,
                      _IOC(_IOC_WRITE, 'K', 14, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_GET_WRAPPER_FD,
                      _IOC(_IOC_WRITE, 'K', 15, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_MANAGE_MARK,
                      _IOC(_IOC_READ | _IOC_WRITE, 'K', 16, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_NUKE_EXT4_SYSFS,
                      _IOC(_IOC_WRITE, 'K', 17, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_MANAGE_TRY_UMOUNT,
...
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_GET_FULL_VERSION,
...
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_DYNAMIC_MANAGER,
                      _IOC(_IOC_READ | _IOC_WRITE, 'K', 103, 0))
// 104 = old get_managers, deprecated
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_GET_MANAGERS,
                      _IOC(_IOC_READ | _IOC_WRITE, 'K', 105, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_KPM,
                      _IOC(_IOC_READ | _IOC_WRITE, 'K', 200, 0))
// KSUN
#ifdef CONFIG_KSU_MULTI_MANAGER_SUPPORT
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_GET_HOOK_MODE,
                      _IOC(_IOC_READ, 'K', 98, 0))
DEFINE_KSU_UAPI_CONST(__u32, KSU_IOCTL_GET_VERSION_TAG,
